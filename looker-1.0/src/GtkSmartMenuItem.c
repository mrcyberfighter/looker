...
struct _GtkSmartMenuItemPrivate {
...
static void gtk_smart_menu_item_dispose(GObject *object) ;
...
static void gtk_smart_menu_item_class_init(GtkSmartMenuItemClass *class) {
  ...
  G_OBJECT_CLASS(class)->dispose = gtk_smart_menu_item_dispose ;
...
static void gtk_smart_menu_item_init(GtkSmartMenuItem *smart_menu_item) {
  ...
  priv->radio_list  = NULL ;  /** The menu accelerator label  **/
}
...
static void gtk_smart_menu_item_dispose(GObject *object) {
  ...
  priv->image = g_object_ref_sink(G_OBJECT(priv->image)) ;

  priv->label = g_object_ref_sink(G_OBJECT(priv->label)) ;

  priv->accel_label = g_object_ref_sink(G_OBJECT(priv->accel_label)) ;

  priv->menuitem = g_object_ref_sink(G_OBJECT(priv->menuitem)) ;

  G_OBJECT_CLASS(gtk_smart_menu_item_parent_class)->dispose(object);
...
GtkWidget* gtk_smart_radio_menu_item_new_all(const gchar *label, const gchar *icon_filepath, GtkAccelGroup *accel_group, const GdkModifierType accel_modifier, const guint accel_key, GtkWidget *widget) {
  ...
  if (widget == NULL) {
    ...
    /** Initialize private data. **/
    priv->menuitem = gtk_radio_menu_item_new(priv->radio_list) ; /** The menu_item **/

  }
  else {

    priv->menuitem = gtk_radio_menu_item_new_from_widget(GTK_RADIO_MENU_ITEM(widget)) ;
...
const gchar *gtk_smart_menu_item_get_label_text(GtkWidget *smart_menu_item) {
...
#if 0
void gtk_smart_menu_item_set_image_from_file(GtkSmartMenuItem *smart_menu_item, const gchar *icon_filepath) {
  ...
  if (g_file_test(icon_filepath, G_FILE_TEST_IS_REGULAR)) {
    ...
    GtkWidget *image = gtk_smart_menu_item_get_image(GTK_WIDGET(smart_menu_item)) ;
    ...
    image = gtk_image_new_from_file(icon_filepath) ;
